...
func (v *errorTxNotFoundResponse) Do(req *http.Request) (*http.Response, error) {
	...
	for _, chain := range getTransactionBlockchains {
		if strings.Contains(req.Host, chain.BlockBookURL()) && strings.Contains(req.URL.String(), "tx/"+testBitcoinTxID) {
			resp.StatusCode = http.StatusBadRequest
			resp.Body = ioutil.NopCloser(bytes.NewBuffer([]byte(`{"error": "Transaction '` + testBitcoinTxID + `' not found"}`)))
			return resp, nil
		}
	}
